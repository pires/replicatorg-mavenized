...
public class MachineLoaderTest {
	...
	public class TestMachineListener implements MachineListener {
		...
		// LinkedBlockingQueue<MachineToolStatusEvent>
		// machineToolStatusEventQueue;
		...
		synchronized MachineStateChangeEvent getMachineStateChangedEvent(
		        int timeout) {
			try {
				return machineStateChangeEventQueue.poll(timeout,
	...
	public void testGetMachine() {
		...
		assertNull(loader.getMachineInterface());
	}
	...
	public void testLoad() {
		...
		// Now, try loading another garbage machine on top of the known good
		// one.
		assertNull(loader.getMachineInterface("fake machine"));
	...
	public void testConnectionInvalidPort() {
		...
		// Make sure that we get them both back in order, within a reasonable
		// time.
		...
		// Finally, make sure that the query interface agrees we aren't
		// connected
		assertFalse(loader.isConnected());
	...
	public void testConnectionKnownGoodPort() {
		...
		for (int i = 0; i < 1; i++) {
			...
			assertNotNull(loader
			        .getMachineInterface("Thingomatic w/ HBP and Stepstruder MK6"));
	...
	public void testDisconnect() {
		...
		// Unload the machine. Since we're not attached, there shouldn't be
		// another message.
		loader.unload();
